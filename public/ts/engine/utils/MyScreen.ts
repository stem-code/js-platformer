class MyScreen { // equivalent of static Class in Java
    ...
    public static ensureScreenBounds = (x: number, width: number) => { // ensure that platform remains within the screen dimensions (bigger than 0, but not further than the screenWidth)
        if (x < 0) { return 0; }
